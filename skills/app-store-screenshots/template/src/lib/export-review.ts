...
export function reviewExport(
  state: ProjectState,
  imageFailed: (path: string) => boolean = () => false,
  sizeOf: (
    path: string,
  ...
  for (const [index, slide] of slides.entries()) {
    ...
    const contrast = backgroundContrast(
      background,
      backgroundText(slide, theme),
    );
    if (contrast !== null && contrast < 4.5) {
      issues.push({
        ...base,
        message: `Screen ${index + 1}: increase headline contrast in Background or Brand settings (target 4.5:1).`,
      });
    }
    for (const locale of state.locales) {
      const prefix = `Screen ${index + 1} · ${locale.toUpperCase()}`;
      const add = (message: string) =>
        issues.push({ ...base, locale, message: `${prefix}: ${message}` });
      if (!slide.headline[locale]?.trim())
        add("add the headline in this language.");
      if (
        Object.values(slide.label).some((value) => value?.trim()) &&
        !slide.label[locale]?.trim()
      ) {
        add("translate the label or remove it from every language.");
      }
      for (const element of slide.textElements || []) {
        if (!element.text[locale]?.trim())
          add(
            "add the extra text in this language or remove the empty text element.",
          );
      }
      for (const element of slide.elements || []) {
        if (element.hidden) continue;
        const checkImage = (raw: string, label: string) => {
          if (!raw) {
            add(`add the ${label} for “${element.name}”.`);
            return false;
          }
          if (imageFailed(resolveScreenshot(raw, locale))) {
            add(`the ${label} for “${element.name}” could not load.`);
            return false;
          }
          return true;
        };
        if (element.kind === "text" && !element.text[locale]?.trim())
          add(`add the text for “${element.name}” in this language.`);
        if (
          element.kind === "image" ||
          element.kind === "logo" ||
          element.kind === "detail"
        )
          checkImage(element.asset.src, "image");
        if (element.kind === "cards")
          for (const [i, item] of element.items.entries()) {
            checkImage(item.asset.src, `card ${i + 1} image`);
            if (
        ...
        if (element.kind === "device") {
          ...
          if (checkImage(element.src, "screenshot") && ownFrame) {
            const size = sizeOf(resolveScreenshot(element.src, locale));
            if (
              size &&
              Math.abs(
                size.width /
                  size.height /
                  (ownFrame.screen.width / ownFrame.screen.height) -
                  1,
              ) > 0.005
            )
              add(
                `use a screenshot with ${ownFrame.screen.width} × ${ownFrame.screen.height} proportions for “${element.name}”.`,
              );
          }
        }
      }
      ...
      if (background.kind === "image")
        paths.push(["background image", background.image.src]);
      if (!feature && slide.layout === "two-devices")
        paths.push(["back screenshot", slide.screenshotSecondary]);
      if (!feature && slide.layout === "creator")
        paths.push(["creator photo", slide.photo?.src]);
      if (!feature && slide.layout === "content-library") {
        const artworks = slide.artworks || [];
        if (artworks.length < 2 || artworks.length > 4)
          add("use two to four catalog images.");
        for (const [i, art] of artworks.entries())
          paths.push([`catalog image ${i + 1}`, art.src]);
      }
      for (const [label, raw] of paths) {
        if (!raw) add(`add the ${label}.`);
        else if (imageFailed(resolveScreenshot(raw, locale)))
          add(`the ${label} could not load. Replace it or check its path.`);
        else if (
          frame &&
          (label === "screenshot" || label === "back screenshot")
        ) {
          const size = sizeOf(resolveScreenshot(raw, locale));
          const expected = frame.screen.width / frame.screen.height;
          if (
            size &&
            Math.abs(size.width / size.height / expected - 1) > 0.005
          ) {
            add(
              `the ${label} is ${size.width} × ${size.height}. Use a capture with ${frame.screen.width} × ${frame.screen.height} proportions for this frame.`,
            );
          }
        }
      }
